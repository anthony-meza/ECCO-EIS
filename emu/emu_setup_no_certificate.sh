...
# REPO_URL="https://github.com/ECCO-GROUP/ECCO-EIS.git"
# BRANCH_NAME="main"

REPO_URL="https://github.com/anthony-meza/ECCO-EIS.git"
BRANCH_NAME="emu-certificate-and-sbatch"
...
# ***************************************
# 0) Install temporary certificates 

echo "Downloading local CA bundle..."
mkdir -p ~/.certs && wget -q -O ~/.certs/cacert.pem https://curl.se/ca/cacert.pem

[ -f ~/.wgetrc ] && cp ~/.wgetrc ~/.wgetrc.bak && echo "Backed up existing .wgetrc"
echo "ca_certificate = $HOME/.certs/cacert.pem" > ~/.wgetrc
echo ".wgetrc updated with local CA bundle"
...
[ -f ~/.wgetrc.bak ] && mv ~/.wgetrc.bak ~/.wgetrc && echo "Restored original .wgetrc" || { rm ~/.wgetrc && echo "Removed temporary .wgetrc"; }
